...
//
const char* const _id = "$Id: TxDepthTest.cpp,v 1.2 2017/10/24 11:45:20 klein Exp klein $";
...
static std::ostream& printTime(std::ostream& os)
{
  ...
  if (tmp == NULL) { throw strerror(errno); } 

  if (strftime(buf, sizeof(buf), format, tmp) == 0) throw "strftime failure";
...
static std::ostream& printError(TPU_HANDLE tpu, std::ostream& os)
{
  ...
  switch(ec)
  {
    case NGS_NO_ERROR: os << "No Error"; break;
    case NGS_NO_NETWORK_SOCKET_OBJECT: os << "No Socket"; break;
    case NGS_NO_CONNECTION_WITH_TPU: os << "No Connection"; break;
    case NGS_ALREADY_CONNECTED: os << "Already Connected"; break;
    case NGS_INVALID_IP_ADDRESS: os << "Invalid IpAddr"; break;
    case NGS_REQUIRES_A_MASTER_CONNECTION: os << "Requres Master"; break;
    case NGS_MASTER_ALREADY_CONNECTED: os << "Already Master Connected"; break;
    case NGS_GETHOSTBYNAME_ERROR: os << "GetHostByName Error"; break;
    case NGS_COMMAND_HANDSHAKE_ERROR: os << "Command Handshake Error"; break;
    case NGS_COMMAND_NOT_SUPPORTTED_BY_CURRENT_PROTOCOL: os << "Command Not Supported"; break;
    case NGS_SEND_COMMAND_FAILURE: os << "Send Failure"; break;
    case NGS_RECEIVE_COMMAND_FAILURE: os << "Receive Failure"; break;
    case NGS_TPU_REPORTS_COMMAND_FAILED: os << "TPU Reports Failure"; break;
    case NGS_UNKNOWN_DATA_PAGE_VERSION: os << "Unknown Data Page Version"; break;
    case NGS_SDFX_RECORD_TYPE_UNKNOWN: os << "Unknown SDFX Type"; break;
    case NGS_SDFX_RECEIVE_BUFFER_TOO_SMALL: os << "SDRX Receive Buffer To Small"; break;
    case NGS_SDFX_HEADER_VERSION_UNKNOWN: os << "SDFX Header Version Unknown"; break;
    case NGS_SDFX_RECORD_VERSION_UNKNOWN: os << "SDFX Record Version Unknown"; break;
    default: os << "Unknown Error Code: " << ec; break;
  }
  return os;
}
...
{
  out << "Depth Settings: "
    << "f(" << d.depthValueRaw << ") = " << d.depthValue
    << ", minDepthLP: " << d.minDepthLowPower
    << ", minDepthHP: " << d.minDepthHighPower
    << ", stat: 0x" << std::hex << std::setw(8) << std::setfill('0') << d.depthStatus
    << std::dec
    ;
  return out;
}
...
{
  std::cerr << "Usage: " << name << "[-h --host hostname]" << std::endl;
...
public:
  ...
  const int execute()
  {
    ...
    while (!shutdown)
    {
      try
      {
        // get the record, handle it, throws on failure
        handleDepth(getDepth());
...
lowerAlarm:
  ...
  void handleDepth(const DepthSettings_t& d)
  {
